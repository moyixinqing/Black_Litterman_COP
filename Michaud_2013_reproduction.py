...
Sigma = np.array([0.054, 0.070, 0.190, 0.244, 0.215,
                  0.244, 0.208, 0.149]).reshape((8, 1))
hPi = np.array([0.032, 0.030, 0.046, 0.105, 0.064,
                0.105, 0.095, 0.085]).reshape((8, 1))
refPi = np.array([0.022, 0.026, 0.096, 0.109, 0.086,
                  0.078, 0.100, 0.085]).reshape((8, 1))
refPi2 = np.array([0.022, 0.026, 0.094, 0.083, 0.063, 0.064, 0.087, 0.092])
refW2 = np.array([0.20, 0.20, 0.06, 0, 0.015, 0.06,
                  0.015, 0.45]).reshape((8, 1))
miW = np.array([0.23, 0.19, 0.099, 0.043, 0.047,
                0.066, 0.054, 0.262]).reshape((8, 1))
assets = [
    'Euro Bonds',
    'US Bonds   ',
    'Canada   ',
...
print('Return\t\t{:.2f}\t\t\t\t\t{:.2f}\t\t\t\t\t{:.2f}\n'.format(
    100 * np.dot(weq, pi.T), 100 * np.dot(w1, Er1.T), 100 * np.dot(w.T, Er)))
print(
    'Risk\t\t{:.2f}\t\t\t\t\t{:.2f}\t\t\t\t\t{:.2f}\n'.format(
        100 *
        np.sqrt(
